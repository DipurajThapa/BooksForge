...
pub async fn save_diagnostic_bundle(
    ...
    tokio::task::spawn_blocking(move || -> Result<SaveDiagnosticBundleResult, BooksForgeError> {
                ...
                if let Ok(rd) = std::fs::read_dir(dir) {
                    for entry in rd.flatten() {
                        ...
                        let name = path.file_name()
                            .and_then(|s| s.to_str())
                            .unwrap_or("");
                        if !name.starts_with("booksforge") { continue; }
                        let raw = match std::fs::read_to_string(&path) {
                            Ok(s)  => s,
                            Err(_) => continue,
                        };
                        let redacted: String = raw.lines()
                            .map(redact_line)
                            .collect::<Vec<_>>()
                            .join("\n");
                        ...
                        zip.write_all(redacted.as_bytes())
                            .map_err(|e| BooksForgeError::internal(format!("write log: {e}")))?;
                        log_count += 1;
                    }
                }
            }
...
fn build_manifest(
    app_version: &str,
    os:          &str,
    project:     Option<&ProjectMetaSnapshot>,
) -> String {
