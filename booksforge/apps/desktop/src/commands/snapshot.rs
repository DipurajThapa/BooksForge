...
async fn resolve_service(
    ...
    let project = project.ok_or_else(|| BooksForgeError::internal("no project is open".to_owned()))?;
    let storage: Arc<dyn StorageRepository> = project.storage.clone();
    let fs:      Arc<dyn BundleFilesystem>  = Arc::new(OsFilesystem);
    Ok(Arc::new(SnapshotService::new(storage, fs, project.bundle.clone())))
}
...
fn parse_scope(s: &str) -> Result<SnapshotScope, BooksForgeError> {
    match s {
        ...
        other => Err(BooksForgeError::validation(format!("invalid scope: {other}"))),
    }
}
...
fn parse_trigger(s: &str) -> Result<SnapshotTrigger, BooksForgeError> {
    match s {
        ...
        other => Err(BooksForgeError::validation(format!("invalid trigger: {other}"))),
    }
}
...
fn parse_ulid(s: &str, label: &str) -> Result<Ulid, BooksForgeError> {
    Ulid::from_string(s).map_err(|e| BooksForgeError::validation(format!("invalid {label} ULID: {e}")))
}
...
pub async fn snapshot_create(
    ...
    let scope_id = input.scope_id.as_deref().map(|s| parse_ulid(s, "scope_id")).transpose()?;
