...
import { gatesEnabled, setGatesEnabled } from "../lib/workflowGates";
...
export default function SettingsPanel({ onClose }: Props) {
  ...
  // Phase 5B — book-kind picker overlay state.
  const [showBookKindPicker, setShowBookKindPicker] = useState(false);

  // Phase 9B — workflow approval gates ("advanced mode" off them off).
  const [workflowGates, setWorkflowGates] = useState<boolean>(() => gatesEnabled());
  ...
  async function handleBundle() {
    ...
    try {
      ...
      setBundleResult(errorMessage(e));
    } finally {
      setBundling(false);
    }
  }
  ...
  return (
    <div style={s.overlay} role="presentation">
      <div {...dialogProps} style={s.dialog}>
        ...
        <div style={s.body}>
          {/* ── Phase 5B — Project classification ── */}
          <section style={s.section}>
            <h4 style={s.sectionTitle}>Book kind</h4>
            <p style={s.sectionBlurb}>
              Drives the drafter prompts, polish-stack ordering, and rubric
              weights. Change this if your project has shifted genres.
            </p>
            <button
              style={s.actionBtn}
              onClick={() => setShowBookKindPicker(true)}
            >
              Change book kind…
            </button>
          </section>

          {/* ── Phase 9 — Workflow approval gates ── */}
          <section style={s.section}>
            <h4 style={s.sectionTitle}>Workflow approval gates</h4>
            <p style={s.sectionBlurb}>
              Four review checkpoints between agent stages — topic, plan, character/world
              bibles, and pre-final-polish. Pause for your approval before drift
              propagates into chapters of prose. Turn off for "advanced mode" — every
              gate auto-approves.
            </p>
            <Toggle
              label="Use approval gates"
              hint="Default: on. Open the Workflow guide from the toolbar to step through them."
              value={workflowGates}
              onChange={(v) => { setWorkflowGates(v); setGatesEnabled(v); }}
            />
          </section>
      ...
      {/* Phase 5B — book-kind picker overlay rendered above the panel
          when the user clicks "Change book kind…". The overlay's mode
          is "settings" so it's dismissible (vs. "onboarding" mode in
          App.tsx which forces the user to pick). */}
      {showBookKindPicker && (
        <BookKindOverlay
          mode="settings"
          // Reading the current kind would require a project-state hook
          // we don't have here; null is acceptable — the overlay just
          // doesn't pre-select a card.
          currentKind={null}
          onSaved={() => setShowBookKindPicker(false)}
          onClose={() => setShowBookKindPicker(false)}
        />
      )}
    </div>
  );
