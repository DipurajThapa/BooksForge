/**
 * Copyeditor agent panel — runs the copyeditor against the active scene
 * and lists the proposed edits with per-edit Accept buttons.  Each Accept
 * dispatches `agent_apply_copyedit` which takes a pre-edit snapshot,
 * mutates the scene, and inserts an `agent_applied_edits` ledger row.
 */
...
export default function CopyeditPanel({ projectId, sceneId, model, onClose }: Props) {
  ...
  const proposals: CopyeditProposals | null = (() => {
    if (!result?.proposal_json) return null;
    try { return JSON.parse(result.proposal_json) as CopyeditProposals; }
    catch { return null; }
  })();
  ...
  async function handleRun() {
    ...
    setResult(null);
    try {
      const input: RunCopyeditInput = {
        project_id: projectId,
        node_id:    sceneId,
        model,
        high_confidence_mode: highConf,
      };
      const r = await ipc.agentRunCopyedit(input);
      setResult(r);
    } catch (e) {
  ...
  async function handleAccept(idx: number) {
    ...
    try {
      const r = await ipc.agentApplyCopyedit({
  ...
  // Apply every edit in `category` that hasn't been actioned yet.
  // Edits are applied serially because each one shifts subsequent
  // ranges — the apply path on the Rust side recomputes ranges after
  // each accept, and we mirror that by awaiting each call in turn.
  async function handleAcceptCategory(category: string) {
    if (!proposals) return;
    const indices = proposals.edits
      .map((e, i) => ({ e, i }))
      .filter(({ e, i }) => e.category === category && !accepted[i])
      .map(({ i }) => i);
  ...
  function categoriesPresent(): { name: string; total: number; pending: number }[] {
    if (!proposals) return [];
    const m = new Map<string, { total: number; pending: number }>();
    proposals.edits.forEach((e, i) => {
      const cur = m.get(e.category) ?? { total: 0, pending: 0 };
      cur.total += 1;
      if (!accepted[i]) cur.pending += 1;
    ...
    <div style={s.overlay} role="presentation">
      <div {...dialogProps} style={s.dialog}>
        <header style={s.header}>
          <strong id={titleId}>Copyeditor</strong>
          <button style={s.close} onClick={onClose} aria-label="Close copyeditor panel">✕</button>
        </header>
        ...
        <div style={s.controls}>
          <label style={s.checkbox}>
            <input
              type="checkbox"
              checked={highConf}
              onChange={e => setHighConf(e.target.checked)}
            />
            High-confidence mode (more peer reviewers)
          </label>
          <button
            style={s.runBtn}
            onClick={handleRun}
            disabled={running || !sceneId}
          >
        ...
        {result && (
          <div style={s.body}>
            ...
            {proposals && (
              <>
                <div style={s.summary}>{proposals.summary}</div>
                {categoriesPresent().length > 0 && (
                  <div style={s.catBar}>
                    <span style={s.catBarLabel}>Accept all by category:</span>
                    {categoriesPresent().map(c => (
                      <button
                        key={c.name}
                        style={{
                          ...s.catBtn,
                          opacity: c.pending === 0 ? 0.5 : 1,
                        }}
                        disabled={c.pending === 0}
                        onClick={() => handleAcceptCategory(c.name)}
                        title={`${c.pending} pending of ${c.total}`}
                      >
                        {c.name} ({c.pending}/{c.total})
                      </button>
                    ))}
                  </div>
                )}
                    ...
                    return (
                      <li key={i} style={s.editRow}>
                        <div style={s.editHead}>
                          <span style={s.cat}>{e.category}</span>
...
const s: Record<string, React.CSSProperties> = {
  ...
  summary:  { padding: 10, background: "var(--color-bg)", borderRadius: 4, fontSize: 13 },
  ...
  cat:      { padding: "1px 6px", border: "1px solid var(--color-border)", borderRadius: 3, fontSize: 11, textTransform: "uppercase" },
  ...
  catBar:   { display: "flex", flexWrap: "wrap", gap: 6, alignItems: "baseline" },
  catBarLabel: { fontSize: 12, opacity: 0.75, marginRight: 4 },
  catBtn:   { padding: "3px 8px", border: "1px solid var(--color-border)", borderRadius: 3, cursor: "pointer", background: "var(--color-bg)", fontSize: 11, textTransform: "uppercase" },
};
