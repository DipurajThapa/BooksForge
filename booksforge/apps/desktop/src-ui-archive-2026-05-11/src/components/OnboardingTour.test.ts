...
describe("OnboardingTour storage helpers", () => {
  ...
  it("survives a localStorage exception by returning false", () => {
    ...
    // catches any throw and returns the safe default. Spy on the
    // instance method (rather than `Storage.prototype.getItem`) so the
    // stub takes effect regardless of whether `localStorage` is the
    // jsdom-provided `Storage` or our in-memory polyfill from
    // `vitest.setup.ts`.
