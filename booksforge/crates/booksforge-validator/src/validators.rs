...
use crate::{
    is_prose_node, issue, scene_word_count, walk_text_nodes_mut, Validator, ValidatorContext,
};
...
pub static ALL_VALIDATORS: &[Validator] = &[
    Validator {
        id: "double-spaces",
        description: "Two or more spaces between words.",
        run: double_spaces,
        fix: Some(fix_double_spaces),
    },
    Validator {
        id: "trailing-whitespace",
        description: "Lines ending in whitespace.",
        run: trailing_whitespace,
        fix: Some(fix_trailing_whitespace),
    },
    Validator {
        id: "multiple-blank-lines",
        description: "Three or more consecutive blank lines.",
        run: multiple_blank_lines,
        fix: Some(fix_multiple_blank_lines),
    },
    Validator {
        id: "em-dash-style",
        description: "Dash usage inconsistent with the project style book.",
        run: em_dash_style,
        fix: Some(fix_em_dash_style),
    },
    Validator {
        id: "quote-style",
        description: "Smart vs straight quotes mixed in the same paragraph.",
        run: quote_style,
        fix: None,
    },
    Validator {
        id: "unmatched-quotes",
        description: "Odd number of straight or smart quotes in a paragraph.",
        run: unmatched_quotes,
        fix: None,
    },
    Validator {
        id: "ellipsis-form",
        description: "Ellipsis form does not match style-book preference.",
        run: ellipsis_form,
        fix: None,
    },
    Validator {
        id: "heading-hierarchy",
        description: "Heading levels skip a step (e.g. H1 → H3).",
        run: heading_hierarchy,
        fix: None,
    },
    Validator {
        id: "missing-alt-text",
        description: "Image without alt text — accessibility / KDP requirement.",
        run: missing_alt_text,
        fix: None,
    },
    Validator {
        id: "broken-links",
        description: "Markdown link with empty URL.",
        run: broken_links,
        fix: None,
    },
    Validator {
        id: "orphan-chapter",
        description: "Chapter has zero scenes.",
        run: orphan_chapter,
        fix: None,
    },
    Validator {
        id: "very-short-scene",
        description: "Scene under 50 words — likely a stub.",
        run: very_short_scene,
        fix: None,
    },
    Validator {
        id: "very-long-scene",
        description: "Scene over 5000 words — review for splitting.",
        run: very_long_scene,
        fix: None,
    },
    Validator {
        id: "untitled-node",
        description: "Project / part / chapter / scene with empty title.",
        run: untitled_node,
        fix: None,
    },
    Validator {
        id: "ai-tells-detected",
        description: "Phrase flagged by the active vocabulary as an LLM tell.",
        run: ai_tells_detected,
        fix: None,
    },
    Validator {
        id: "vocab-replace-pending",
        description: "Term has a `replace` rule that hasn't been applied to this scene.",
        run: vocab_replace_pending,
        fix: Some(fix_vocab_replace),
    },
    Validator {
        id: "kdp-metadata",
        description: "KDP-eBook metadata gate: title / author / language / ISBN sanity.",
        run: kdp_metadata,
        fix: None,
    },
];
    ...
    for scene in ctx.scenes {
        ...
        if opens != closes {
            out.push(issue(
                ...
                "UMQ02",
                Severity::Warning,
                format!("Smart-quote imbalance: {opens} opening (“) vs {closes} closing (”)."),
                Some(scene.node_id),
                None,
                None,
                false,
...
fn heading_hierarchy(ctx: &ValidatorContext) -> Vec<ValidatorIssue> {
    ...
    for node in ctx.nodes {
        ...
        let valid = match (parent_kind, node.kind) {
            ...
            // A "Front Matter" container legitimately holds nested
            // front-matter pages (Title Page, Copyright, Dedication,
            // Foreword, Epigraph, Table of Contents). Same for
            // "Back Matter" → Epilogue / Acknowledgements / Glossary /
            // About the Author / Index. The shipped GENERIC_NOVEL
            // template uses this composition pattern, and it matches
            // standard publishing convention. Permit both.
            (Some(NodeKind::FrontMatter), NodeKind::FrontMatter) => true,
            (Some(NodeKind::BackMatter), NodeKind::BackMatter) => true,
...
fn orphan_chapter(ctx: &ValidatorContext) -> Vec<ValidatorIssue> {
    ...
    for chapter in ctx.nodes.iter().filter(|n| n.kind == NodeKind::Chapter) {
        let has_scene = ctx
            .nodes
            .iter()
            .any(|n| n.parent_id == Some(chapter.id) && is_prose_node(n.kind));
        if !has_scene {
            out.push(issue(
                "orphan-chapter",
                "ORP01",
...
fn ai_tells_detected(ctx: &ValidatorContext) -> Vec<ValidatorIssue> {
    ...
    let avoid_terms: Vec<(&str, &str, Option<&str>)> = resolved
        .iter()
    ...
    for scene in ctx.scenes {
        ...
        for (term, layer, rationale) in &avoid_terms {
            if let Some(idx) = lower.find(term) {
                let kind_label = if *layer == "ai_tells" {
                    "AI tell"
                } else {
                    "vocab rule"
                };
                let mut msg = format!("{kind_label} ({layer}): \"{term}\" — replace or rephrase.",);
...
fn vocab_replace_pending(ctx: &ValidatorContext) -> Vec<ValidatorIssue> {
    let mut out = Vec::new();
    let resolved = booksforge_domain::resolve_vocab(ctx.vocab, ctx.active_vocab_layers);
    let replace_rules: Vec<(&str, &str, &str)> = resolved
        .iter()
        .filter(|e| e.kind == EntryKind::Replace && e.replacement.is_some())
        .map(|e| {
            (
                e.term.as_str(),
                e.replacement.as_deref().unwrap_or_default(),
                e.layer.as_str(),
            )
        })
        .collect();
    ...
    for scene in ctx.scenes {
        ...
        for (term, replacement, layer) in &replace_rules {
            if let Some(idx) = lower.find(term) {
                out.push(issue(
                    "vocab-replace-pending",
                    "VRP01",
                    Severity::Warning,
                    format!("Vocabulary rule ({layer}): replace \"{term}\" → \"{replacement}\".",),
                    Some(scene.node_id),
                    Some(idx as u32),
                    Some((idx + term.len()) as u32),
                    true,
    ...
    if let Some(raw) = &meta.isbn {
        ...
        if !(cleaned.len() == 10 || cleaned.len() == 13) {
            out.push(issue(
                ...
                Severity::Error,
                format!(
                    "KDP: ISBN '{raw}' must be 10 or 13 digits (got {} useful chars).",
                    cleaned.len()
                ),
                None,
                None,
...
mod tests {
    ...
    fn ctx<'a>(
        ...
        scenes: &'a [SceneText],
        style: &'a StyleBook,
        vocab: &'a [booksforge_domain::VocabEntry],
        layers: &'a [&'a str],
    ) -> ValidatorContext<'a> {
        ValidatorContext {
            nodes,
    ...
    #[test]
    fn heading_hierarchy_allows_nested_front_and_back_matter() {
        // Mirrors the GENERIC_NOVEL template shape — a "Front Matter"
        // container holds Title Page / Dedication, and a "Back Matter"
        // container holds Epilogue / Acknowledgements. This is the
        // standard publishing convention; the validator must not flag
        // it as a hierarchy violation.
        let project = Ulid::new();
        let fm = Ulid::new();
        let title_page = Ulid::new();
        let dedication = Ulid::new();
        let bm = Ulid::new();
        let epilogue = Ulid::new();
        let acks = Ulid::new();
        let nodes = vec![
            node(project, None, NodeKind::Project, "Book"),
            node(fm, Some(project), NodeKind::FrontMatter, "Front Matter"),
            node(title_page, Some(fm), NodeKind::FrontMatter, "Title Page"),
            node(dedication, Some(fm), NodeKind::FrontMatter, "Dedication"),
            node(bm, Some(project), NodeKind::BackMatter, "Back Matter"),
            node(epilogue, Some(bm), NodeKind::BackMatter, "Epilogue"),
            node(acks, Some(bm), NodeKind::BackMatter, "Acknowledgements"),
        ];
        let style = StyleBook::default();
        let issues = heading_hierarchy(&ctx(&nodes, &[], &style, &[], &[]));
        assert!(issues.is_empty(), "expected no HRC issues, got {issues:?}");
    }
    ...
    fn kdp_metadata_warns_when_no_meta_supplied() {
        ...
        assert!(
            kdp_metadata(&ctx).is_empty(),
            "no meta = no checks (graceful)"
        );
    }
    ...
    fn fix_trailing_whitespace_strips_line_endings() {
        ...
        let count =
            fix_trailing_whitespace(&mut doc, &ctx(&[], &[], &StyleBook::default(), &[], &[]));
        assert_eq!(count, 1);
    ...
    fn fix_multiple_blank_lines_collapses_to_one() {
        let mut doc = paragraph_doc("alpha\n\n\n\nbeta");
        let count =
            fix_multiple_blank_lines(&mut doc, &ctx(&[], &[], &StyleBook::default(), &[], &[]));
        assert_eq!(count, 1);
        assert_eq!(first_paragraph_text(&doc), "alpha\n\nbeta");
    }
