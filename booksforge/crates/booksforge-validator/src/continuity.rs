...
use booksforge_domain::{
    ContinuityEvidence, ContinuityFinding, ContinuityKind, Entity, Severity,
};
...
pub fn detect_name_drift(
    ...
    while i < chars.len() {
        ...
        if !token.chars().next().map(|c| c.is_uppercase()).unwrap_or(false) { continue; }
        if token.chars().all(|c| c.is_uppercase()) { continue; }
        if sentence_initial { continue; }
...
fn levenshtein_capped(a: &str, b: &str, cap: usize) -> usize {
    ...
    if a.len().abs_diff(b.len()) > cap { return cap + 1; }
...
pub fn detect_pov_drift(
    ...
    let count = |needles: &[&str]| -> usize {
        needles.iter().map(|n| count_word(&lower, n)).sum()
    };
    let first_singular = count(&["i", "me", "my", "mine", "myself"]);
    let third_singular = count(&["he", "him", "his", "she", "her", "hers", "they", "them", "their", "theirs"]);
    let total = first_singular + third_singular;
    if total < 5 { return Vec::new(); }    // too short to judge
...
pub fn detect_tense_drift(node_id: &str, scene_text: &str) -> Vec<ContinuityFinding> {
    ...
    let signals: Vec<(f64, f64)> = paragraphs.iter().map(|(_, p)| {
        let words: Vec<&str> = p.split_whitespace().collect();
        if words.is_empty() { return (0.0, 0.0); }
        let mut past = 0;
        let mut pres = 0;
        for w in &words {
...
const COMMON_ED_NON_PAST: &[&str] = &[
    "indeed", "embed", "exceed", "succeed", "proceed", "feed", "need", "seed",
    "speed", "deed", "weed", "creed", "freed", "agreed", "decreed", "guaranteed",
    "red", "bed", "fed", "led",
];
const COMMON_PRESENT_S: &[&str] = &[
    "says", "thinks", "knows", "feels", "wants", "needs", "looks", "sees", "hears",
    "walks", "runs", "stands", "sits", "comes", "goes", "lives", "moves", "works",
    "smiles", "laughs", "shouts", "speaks", "writes", "reads", "holds", "takes",
    "makes", "gives", "calls", "asks", "answers", "tells", "explains",
];
...
mod tests {
    ...
    fn entity(name: &str, aliases: &[&str]) -> Entity {
        Entity {
            id: Ulid::new(), kind: EntityKind::Character,
            name: name.to_owned(),
            aliases: aliases.iter().map(|s| (*s).to_string()).collect(),
            fields_json: serde_json::json!({}),
            notes: String::new(), created_at: Utc::now(), updated_at: Utc::now(),
            deleted_at: None,
        }
    }
    ...
    fn name_drift_marks_close_matches_unambiguous() {
        ...
        let f = detect_name_drift("01HX", scene, &bible);
        let eliotg = f.iter().find(|x| x.evidence[0].excerpt == "Eliotg").expect("Eliotg flagged");
