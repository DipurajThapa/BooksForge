...
fn clean_manuscript_yields_pass_gate() {
    ...
    let scenes = vec![
        scene(scene_id, "A clean opening paragraph that goes for a while \
...
fn manuscript_with_errors_blocks_export() {
    ...
    let scenes = vec![
        scene(bad_scene, "Look at this picture: ![](image.png) and beware  \
                          extra spaces."),
    ];
    let style = StyleBook::default();
    let ctx = ValidatorContext {
...
fn ai_tell_in_active_layer_warns_via_vocab() {
    ...
    let scenes = vec![
        scene(scene_id, "We will delve into the data and showcase the results."),
    ];
    let style = StyleBook::default();
    let vocab = vec![
        VocabEntry::new("ai_tells", "delve",     EntryKind::Avoid, EntrySource::Starter),
        VocabEntry::new("ai_tells", "showcase",  EntryKind::Avoid, EntrySource::Starter),
    ];
    let ctx = ValidatorContext {
        nodes: &nodes,
        scenes: &scenes,
    ...
    let report = run_all_validators(&ctx);
    let ai_issues: Vec<_> = report.issues.iter()
        .filter(|i| i.validator_id == "ai-tells-detected")
        .collect();
    assert!(ai_issues.len() >= 2, "expected ≥2 ai-tells issues, got {}", ai_issues.len());
