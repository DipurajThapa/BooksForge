...
pub fn run_tier1(
    ...
    for v in spec.validators {
        checks.push(cross_cutting::run_validator(
            *v, raw_output, parsed, entity_bible, spec.id, proposed_memory_scopes,
    ...
    let summary = match verdict {
        ...
        ValidationVerdict::Warn  => {
            let n = checks.iter().filter(|c| matches!(c.outcome, booksforge_domain::ValidationOutcome::Warn)).count();
            format!("{n} cross-cutting check(s) raised warnings — review before applying.")
        }
        ValidationVerdict::Block => {
            let n = checks.iter().filter(|c| matches!(c.outcome, booksforge_domain::ValidationOutcome::Fail)).count();
            format!("{n} cross-cutting check(s) FAILED — proposal blocked.")
        }
    };
    ProposalValidation { verdict, checks, summary, tier_2_ran: false }
}
...
mod tests {
    ...
    use booksforge_agents::{
        AgentSpec, ContextBudget, CrossCuttingValidator, ModelFamily, ModelPreference,
    ...
    fn dummy_spec() -> AgentSpec {
        AgentSpec {
            id: "copyeditor", name: "Copyeditor", purpose: "p",
            input_schema_id: "X", output_schema_id: "Y",
            prompt_template: PromptTemplateId::new("copyeditor", "v1"),
            model_preference: ModelPreference {
                family: ModelFamily::AnyInstruct, min_size: ModelSizeHint::Medium,
            },
            pinned_model: None,
            context_budget: ContextBudget { max_context_tokens: 4_000, max_output_tokens: 2_000 },
            validators: &[
            ...
            failure_modes: &[],
            when_to_run: WhenToRun::OnDemand, user_gate: UserGate::Required,
        }
    }
    ...
    fn warns_on_chain_of_thought_leak() {
        ...
        let out    = run_tier1(&spec, raw, &parsed, &[], &[], None, None);
        assert!(matches!(out.verdict, ValidationVerdict::Warn | ValidationVerdict::Block));
    }
}
