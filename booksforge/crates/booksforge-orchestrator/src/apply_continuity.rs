...
use booksforge_domain::{
    flat_text_to_pm_doc, pm_doc_to_text, AppliedEditKind, ContinuityFixKind, ContinuityFixScope,
    ContinuityReport, MemoryEntry, MemoryScope, NodeKind, SceneContent, SnapshotScope,
};
...
impl Orchestrator {
    ...
    pub async fn apply_continuity_fix(
        ...
        let report: ContinuityReport = serde_json::from_str(&raw).map_err(|e| {
        ...
        match finding.proposed_fix.kind {
            ...
            ContinuityFixKind::Rename => {
                ...
                apply_rename(
                    ...
                    &from,
                    &to,
                    finding.proposed_fix.scope,
                    finding
                        .evidence
                        .iter()
                        .filter_map(|e| Ulid::from_string(&e.node_id).ok())
                        .collect(),
                )
                .await
            }
...
async fn apply_rename(
    ...
    let (snap_scope, scope_id, candidate_scenes) = match scope {
        ...
        ContinuityFixScope::Chapter => {
            ...
            let nodes = storage
                ...
                .await
                .map_err(|e| OrchestratorError::Storage(e.to_string()))?;
            let chapter_id = evidence_node_ids.first().and_then(|nid| {
    ...
    let pre = snapshot
        .pre_agent_edit_snapshot(
            ...
            scope_id,
            Some(format!(
                "pre-continuity-rename task {task_id} finding {finding_index} ({from} -> {to})"
            )),
        )
        .await
        .map_err(|e| OrchestratorError::Storage(e.to_string()))?;
...
async fn apply_annotate(
    ...
    let pre = snapshot
        .pre_agent_edit_snapshot(
            ...
            Some(project_id),
            Some(format!(
                "pre-continuity-annotate task {task_id} finding {finding_index}"
            )),
        )
        .await
        .map_err(|e| OrchestratorError::Storage(e.to_string()))?;
...
pub fn whole_word_replace(haystack: &str, from: &str, to: &str) -> String {
    if from.is_empty() || haystack.is_empty() {
...
mod tests {
    ...
    fn whole_word_replaces_only_full_matches() {
        assert_eq!(
            whole_word_replace("Anna and Annapurna", "Anna", "Anya"),
            "Anya and Annapurna"
        );
    }
    ...
    fn whole_word_handles_punctuation_boundaries() {
        assert_eq!(
            whole_word_replace("Anna, Anna's pen.", "Anna", "Anya"),
            "Anya, Anya's pen."
        );
    }
    ...
    fn whole_word_handles_repeated_occurrences() {
        assert_eq!(whole_word_replace("Anna Anna Anna", "Anna", "X"), "X X X");
    }
