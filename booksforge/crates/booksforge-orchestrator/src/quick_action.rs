...
use booksforge_domain::{
    AiCall, AiCallStatus, QuickActionPreset, SnapshotScope,
};
...
impl Orchestrator {
    ...
    pub async fn run_quick_action(
        ...
        let rendered = render(&template_id, &vars).map_err(|e| OrchestratorError::Storage(
            format!("prompt render failed for {tpl_id}: {e}")
        ))?;
        ...
        let result = self.ollama_arc().generate(req, tee_sink, cancel.clone()).await;
        ...
        let (status, error_message, ctx_tok, out_tok) = match &result {
            ...
            Err(booksforge_ollama::OllamaError::Cancelled) => {
                (AiCallStatus::Cancelled, Some("cancelled by user".to_owned()), None, None)
            }
        ...
        storage.ai_call_insert(&row).await.map_err(|e| OrchestratorError::Storage(e.to_string()))?;
    ...
    pub async fn apply_quick_action(
        ...
        let svc: Arc<SnapshotService> = self
            .snapshot()
        ...
        let call = storage.ai_call_get(ai_call_id).await
        ...
        if call.applied_at.is_some() {
            return Err(OrchestratorError::AlreadyApplied { task_id: ai_call_id });
        }
        ...
        let snap = svc
            .pre_ai_snapshot(
                SnapshotScope::Scene,
                Some(call.node_id),
                Some(format!("pre-{} for ai_call {ai_call_id}", call.preset.as_str())),
            )
            .await
            .map_err(|e| OrchestratorError::Storage(e.to_string()))?;

        // 3. Build the new scene_content.pm_doc.
        let existing = storage.load_scene(call.node_id).await
            .map_err(|e| OrchestratorError::Storage(e.to_string()))?;
        let new_text = match op {
            ApplyOp::Replace => accepted_text.clone(),
            ApplyOp::Append => {
                let prev = existing.as_ref().and_then(|s| extract_plain_text(&s.pm_doc)).unwrap_or_default();
                if prev.is_empty() { accepted_text.clone() }
                else { format!("{prev}\n\n{accepted_text}") }
            }
        };
        ...
        storage.ai_call_update_apply(ai_call_id, snap.id, applied_at).await
            .map_err(|e| OrchestratorError::Storage(e.to_string()))?;
...
fn extract_plain_text(pm_doc: &serde_json::Value) -> Option<String> {
    ...
    for block in blocks {
        ...
        if !buf.is_empty() { paragraphs.push(buf); }
    }
