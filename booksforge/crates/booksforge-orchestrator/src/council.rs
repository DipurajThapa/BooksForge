...
use booksforge_domain::{
    peer_reviewers_for, PeerReviewPairing, PeerReviewResult, ProposalValidation, ValidationVerdict,
    VerificationReport,
};
...
pub fn assemble_report(
    ...
    let final_verdict =
        VerificationReport::aggregate_verdicts(&tier_1, tier_2.as_ref(), &peer_reviews);
    VerificationReport {
...
mod tests {
    ...
    fn default_mode_picks_only_default_on_pairings() {
        ...
        assert!(
            p.len() >= 3,
            "default-mode should still pick the three default-on reviewers"
        );
