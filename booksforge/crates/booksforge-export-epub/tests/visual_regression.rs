...
fn epub_chapter_paragraphs_match_editor_preview() {
    ...
    assert!(!preview_chapters.is_empty(), "fixture must produce at least one chapter");

    // Build the EPUB.
    let chapters: Vec<HtmlChapter> = preview_chapters.iter()
        .map(|c| HtmlChapter {
...
fn paragraph_order_is_preserved_across_paths() {
    ...
    let chapters: Vec<HtmlChapter> = preview.iter()
        ...
        .collect();
    let bytes = build_epub_bytes(&EpubPackageInput {
        chapters, metadata: EpubMetadata {
            title: "T".into(), authors: vec!["A".into()], language: "en".into(),
            publisher: None, description: None, isbn: None,
