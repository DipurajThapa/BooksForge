...
impl FormatProfile {
    ...
    pub fn trim_inches(self) -> (&'static str, &'static str) {
        spec(self).trim
    }
    pub fn chapter_starts_recto(self) -> bool {
        spec(self).chapter_starts_recto
    }
    pub fn drop_cap(self) -> bool {
        spec(self).drop_cap
    }

    /// Body font *family* — the short Google Font name.
    pub fn google_body_family(self) -> &'static str {
        spec(self).google_body
    }
    pub fn google_heading_family(self) -> &'static str {
        spec(self).google_heading
    }
    ...
    pub fn body_font_family(self) -> String {
        ...
        if is_sans {
            format!(
                "\"{g}\", \"Source Sans Pro\", \"Helvetica Neue\", Helvetica, Arial, sans-serif"
            )
        } else {
            format!(
                "\"{g}\", \"Adobe Garamond Pro\", Garamond, Georgia, \"Times New Roman\", serif"
            )
        }
    }
    ...
    pub fn body_em(self) -> &'static str {
        spec(self).body_em
    }
    pub fn body_size_pt(self) -> &'static str {
        spec(self).body_size_pt
    }
    pub fn line_height(self) -> &'static str {
        spec(self).line_height
    }
    pub fn scene_break_glyph(self) -> &'static str {
        spec(self).scene_break_glyph
    }

    /// Inline SVG ornament for the scene break.  Empty for `Academic`.
    /// The CSS factory wraps this in a data URI.
    pub fn ornament_svg(self) -> &'static str {
        spec(self).ornament_svg
    }

    pub fn paragraph_indent_em(self) -> &'static str {
        spec(self).paragraph_indent_em
    }
    pub fn pandoc_documentclass(self) -> &'static str {
        spec(self).pandoc_documentclass
    }

    pub fn pandoc_classoption(self) -> &'static str {
        if self.chapter_starts_recto() {
            "twoside,openright"
        } else {
            "twoside"
        }
...
mod tests {
        ...
        for &g in Genre::all() {
            for &p in g.sub_genres() {
                assert_eq!(
                    p.genre(),
                    g,
                    "Genre::{g:?}::sub_genres() lists {p:?} but its genre() returns {:?}",
                    p.genre()
                );
            }
        }
    ...
    fn every_profile_has_a_google_body_font() {
        for &p in ALL_PROFILES {
            assert!(
                GOOGLE_FONT_BUNDLE.contains(&p.google_body_family()),
                "{p:?} body font {} not in GOOGLE_FONT_BUNDLE",
                p.google_body_family()
            );
            assert!(
                GOOGLE_FONT_BUNDLE.contains(&p.google_heading_family()),
                "{p:?} heading font {} not in GOOGLE_FONT_BUNDLE",
                p.google_heading_family()
            );
        }
    }
    ...
    fn every_non_academic_profile_has_an_ornament() {
        for &p in ALL_PROFILES {
            if matches!(p, FormatProfile::Academic) {
                continue;
            }
            assert!(
                !p.ornament_svg().is_empty(),
                "{p:?} should have an ornament SVG"
            );
            assert!(
                p.ornament_svg().contains("<svg"),
                "{p:?} ornament must be SVG"
            );
        }
    }
