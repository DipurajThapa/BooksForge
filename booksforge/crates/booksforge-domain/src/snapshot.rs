...
impl SnapshotTree {
    ...
    pub fn new(entries: Vec<TreeEntry>) -> Self {
        Self { schema_version: Self::CURRENT_SCHEMA_VERSION, entries }
    }
    ...
    for (node_id, (ea, eb)) in by_id {
        match (ea, eb) {
            (None, Some(e)) => out.push(NodeDiff {
                node_id, kind: NodeDiffKind::Added, title: e.title.clone(),
            }),
            (Some(e), None) => out.push(NodeDiff {
                node_id, kind: NodeDiffKind::Removed, title: e.title.clone(),
            }),
            (Some(ea), Some(eb)) if ea != eb => out.push(NodeDiff {
                node_id, kind: NodeDiffKind::Changed, title: eb.title.clone(),
            }),
            _ => {}
        }
    }
...
mod tests {
    ...
    fn diff_detects_added_removed_changed() {
        ...
        assert_eq!(d.len(), 2);
        assert!(d.iter().any(|n| n.node_id == id2 && n.kind == NodeDiffKind::Changed));
        assert!(d.iter().any(|n| n.node_id == id3 && n.kind == NodeDiffKind::Added));
    }
