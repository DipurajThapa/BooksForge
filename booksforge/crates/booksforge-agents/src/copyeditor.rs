...
const FAILURE_MODES: &[FailureMode] = &[
    FailureMode { id: "range-mismatch",      description: "`before` text doesn't match the source at the given range — fabricated position.", recoverable: true  },
    FailureMode { id: "rewrite-not-fix",     description: "Edit changes word count by more than 10% — Copyeditor must not reword.",            recoverable: true  },
    FailureMode { id: "overlap",             description: "Two edits overlap on the same span.",                                               recoverable: true  },
    FailureMode { id: "category-out-of-enum", description: "Edit category is not in the fixed enum.",                                          recoverable: true  },
];
...
pub fn into_domain(out: ModelOutput) -> CopyeditProposals {
    let edits = out.edits.into_iter().map(|e| CopyeditEdit {
    ...
    CopyeditProposals { edits, summary: out.summary }
}
...
pub fn parse_and_validate(
    raw: &str,
    source_text: &str,
) -> Result<CopyeditProposals, String> {
...
mod tests {
    ...
    fn into_domain_translates_categories() {
        let out = ModelOutput {
            edits: vec![ModelEdit {
                ...
                category: "casing".into(), rationale: "title".into(),
            }],
            summary: "x".into(),
        };
    ...
    fn unknown_category_falls_through_to_other() {
        assert!(matches!(parse_category("nonsense"), CopyeditCategory::Other));
    }
}
