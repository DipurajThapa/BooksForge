...
const FAILURE_MODES: &[FailureMode] = &[
    FailureMode { id: "false-alias-flag",   description: "Flagged a known alias as a name drift.",         recoverable: true  },
    FailureMode { id: "missing-evidence",   description: "Finding has no evidence span.",                   recoverable: true  },
    FailureMode { id: "rename-target-empty", description: "Rename fix lacks both `from` and `to`.",         recoverable: true  },
    FailureMode { id: "kind-out-of-enum",   description: "Finding kind is not in the fixed enum.",          recoverable: true  },
];
...
pub fn parse_and_validate(raw: &str) -> Result<ContinuityReport, String> {
    let model: ModelOutput = serde_json::from_str(raw)
...
pub fn into_domain(out: ModelOutput) -> ContinuityReport {
    let findings = out.findings.into_iter().map(|e| ContinuityReportEntry {
        kind:     parse_kind(&e.kind),
        severity: parse_severity(&e.severity),
        evidence: e.evidence.into_iter().map(|ev| ContinuityEvidence {
            node_id:    ev.node_id,
...
mod tests {
    ...
    fn parse_kind_round_trips_known_values() {
        assert!(matches!(parse_kind("name_drift"), ContinuityKind::NameDrift));
    ...
    fn parse_fix_kind_normalises_case() {
        assert!(matches!(parse_fix_kind("Rename"),   ContinuityFixKind::Rename));
        assert!(matches!(parse_fix_kind("ANNOTATE"), ContinuityFixKind::Annotate));
